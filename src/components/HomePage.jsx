...
import { loadRoutes, TripContext, loadTrips, loadTripsAction } from '../store.js'
  ...
  const tripCardsPastDisplay = () => {
    const tripCards = trips.map((trip) => (
      ...
      {getTripStatus(moment(trip.startDate).format('lll'), moment(trip.endDate).format('lll')) === 2 ?
        <Card title={trip.name} body={moment(trip.startDate).format('ll')} onClick={() => {handleDisplay(trip)}}> </Card> : ""
      }
      </div>
      ))
    return tripCards;
